...
- Facturen (augustus 2026): de portal kan nu een echte factuur uitschrijven, printbaar / op te slaan als PDF. Aanleiding: BJAY heeft (nog) geen KVK-inschrijving; een KVK-nummer is geen verplicht factuur-element, dus een factuur kan gewoon zonder. De opbrengst gaat als resultaat uit overige werkzaamheden in de IB-aangifte, en daarom staat er `Geen btw in rekening gebracht.` op de factuur i.p.v. een btw-specificatie. **Alle vaste gegevens staan in `src/lib/invoiceSettings.ts`**: `INVOICE_SENDER` (naam, handelsnaam, adres, e-mail, telefoon, IBAN, plus lege `kvk`/`vatNumber` die pas op de factuur verschijnen zodra ze ingevuld zijn), `PAYMENT_TERM_DAYS` (14) en `VAT_NOTE`. Zodra de inschrijving rond is: dat ene bestand aanpassen (en bij btw-plicht zonder KOR moeten er ook btw-kolommen op het vel terug, zie de comment daar).
