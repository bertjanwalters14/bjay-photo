...
- Event-admin (juni 2026): aanvraag-notificatie toont nu altijd een leesbare naam (gedeelde `readableEventName` in `events.ts`, ook gebruikt door de requestable-route) i.p.v. "undefined" bij events zonder `name`. Op de event-pagina (`/admin/event/[slug]`) staan nu snelacties bovenaan: popup direct aan/uit zetten (instant PATCH) + een teller "X aanvragen (Y nieuw)" met een knop naar de aanvragen-lijst.
