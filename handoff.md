...
src/
  ...
  lib/
    ...
    eventPackages.ts       # pricing-tier + prijs-per-foto ladder
...
- Omzet-tracking (juni 2026): nieuw overzicht **`/admin/revenue`** ("Omzet"-knop op het dashboard naast Bestellingen) dat alleen **ECHT ontvangen geld** optelt. Twee bronnen: (1) event/print-`Order`s met status `paid` of `shipped` (verzonden = al betaald), (2) personal shoots met ingevuld `Client.price` én gezette `Client.paidAt`. `Client` heeft daarvoor een nieuw optioneel `paidAt?: string | null`; op de admin-klantpagina staat bij personal-klanten mét bedrag een groene knop **"Markeer als betaald"** (PATCH `paidAt`, toggle, toont "betaald op <datum>"). Aggregatie server-side in `GET /api/revenue` (admin-only): totaal ontvangen, totaal dit jaar, split orders vs personal, per-maand-overzicht (orders op `createdAt`, personal op `paidAt`), plus een informatieve "nog openstaand" (klant mét bedrag zonder `paidAt`, telt NIET mee; `outstanding.items` geeft per shoot `code`/`name`/`amount`/`type`, op de pagina klikbaar naar `/admin/clients/<code>` met personal/event-labeltje).
- Bedrag + opmerkingen ook voor events (juni 2026): `Client.price`, `Client.personalNote` en `Client.paidAt` gelden nu óók voor event-klanten. Het "Bedrag shoot"- en opmerkingen-veld staan in beide formulieren (`/admin/clients/new` + inline bewerken) voor élk type; bij personal voeden ze de oplever-mail (betaalregel + opening), bij event zijn ze **puur intern** (omzet-tracking + notitie, geen mail — events hebben geen klant-mailadres). De "Markeer als betaald"-knop + bedrag-weergave op de klantpagina tonen nu bij elke klant mét bedrag (was personal-only). In het omzet-overzicht is **"Event shoots"** een **aparte categorie** naast Bestellingen en Personal shoots (`GET /api/revenue` → `eventShoots` + `MonthBucket.eventShoots`); event-shoot-vergoedingen tellen mee als verdiend zodra `paidAt` gezet is. Labels/hulptekst in de forms zijn type-afhankelijk (event: "Opmerkingen (intern)" i.p.v. "Persoonlijk bericht"). Bedragen worden uit de opgeslagen prijs-tekst geparsed met nieuwe helper **`parsePrice()`** in `src/lib/format.ts` (spiegelbeeld van `formatPrice`), totalen getoond met **`formatEuros()`**. Reden createdAt voor orders: orders houden geen betaaldatum bij, alleen `updatedAt`. Bestaande betaalde orders tellen automatisch mee (geen migratie nodig).
- Recente momenten klikbaar (juni 2026): de "Meest recente momenten"-tegels op de homepage van bjay.photo kunnen nu naar een verhaal-pagina linken. `RecentPhoto.href` bestond al in het datamodel (opgeslagen + via `GET /api/recent` met CORS geserveerd) maar werd nergens beheerd of gebruikt. Op `/admin/recent` staat nu **één gedeeld link-veld** ("Link voor alle tegels") i.p.v. per tegel, omdat de vier foto's altijd van één shoot zijn; bij opslaan wordt die `href` op alle vier de slots gezet. **Website-kant** (statische site, aparte repo, deploy via FileZilla): in `index.html` rendert `initRecentPhotos()` een foto mét `href` als `<a class="item" href>` i.p.v. `<div class="item">` (alleen de homepage heeft de dynamische API-gevoede grid met `id="recent-grid"`; de SEO-pagina's gebruiken `.recent-grid` als statische lightbox-galerij en zijn ongemoeid). Let op: deze wijziging vereist dus **twee deploys** — portal via git push, `index.html` via FileZilla.
- Voorwaarden-akkoord via boekingsmail (juni 2026, alleen personal): klant gaat aantoonbaar akkoord op de algemene voorwaarden. `Client` heeft twee nieuwe velden: `bookingMailSentAt` en `termsAcceptedAt`. Flow: (1) admin-klantpagina heeft een knop **"Stuur boekingsbevestiging"** (personal mét e-mail) → `POST /api/clients/[id]/booking-mail` (admin, weigert events) verstuurt via Resend de **boekingsmail** (`sendBookingMail` in `clientMail.ts`: aanhef + shoot/albumnaam + datum + afgesproken bedrag + knop "Bekijk en ga akkoord met de voorwaarden" → `app.bjay.photo/akkoord?code=`) en zet `bookingMailSentAt`. (2) De knop linkt naar de nieuwe **`/akkoord`-pagina** (`src/app/akkoord/page.tsx`): logt in met de code (`POST /api/auth/client`, possessie van code = toegang), toont de boekingssamenvatting + link naar `bjay.photo/voorwaarden`, en een knop "Ik ga akkoord" → `POST /api/clients/[id]/accept-terms` (via `canActAsClient`, idempotent) zet `termsAcceptedAt`. (3) **Galerij-gate**: in `gallery/[clientId]/page.tsx` krijgt een personal-klant die wél een boekingsmail heeft gekregen (`bookingMailSentAt`) maar nog niet akkoord is, **eerst het akkoord-scherm** i.p.v. de foto's — zo gaan niet-klikkers alsnog akkoord wanneer ze hun foto's willen zien. De gate is bewust gekoppeld aan `bookingMailSentAt` zodat **bestaande klanten niet ineens geblokkeerd** worden. Events nooit gegated (gedeelde code met bezoekers). Gedeelde UI: `src/components/TermsAccept.tsx` (gebruikt door de akkoord-pagina én de gate). Admin-klantpagina toont status "Voorwaarden: akkoord op <datum>" / "boekingsmail verstuurd · nog niet akkoord". Boekingsmail staat ook in `/admin/mail-preview`. **Bij akkoord** gaat er een interne kale-tekst-mail naar BJAY's inbox (`sendInternalMail` in `email.ts`, los van `sendBrandedMail`), en op de **klantenlijst (dashboard)** staat naast het Personal-labeltje een groene **"✓ Akkoord"**-badge zodra `termsAcceptedAt` gezet is, of een oranje **"Wacht op akkoord"** als de boekingsmail wel uit is (`bookingMailSentAt`) maar nog geen akkoord. Bewust NIET gebouwd (keuze BJAY): geen reminder-cron, geen handmatige "markeer als akkoord", en niet voor events (daar zit het fotorecht meestal al bij de organisatie en zitten bezoekers in de galerij).
- Betaalverzoek-mail voor events (juni 2026): de organisator van een event kan nu een betaalverzoek per mail krijgen (geen formele factuur — bewust niet, geen KVK/BTW). `Client` heeft `paymentRequestSentAt`. Nieuwe mail `sendPaymentRequestMail` (`clientMail.ts`): aanhef (`contactName` of "Hoi,"), bedankt voor samenwerking bij `<name>`, het afgesproken bedrag (`price`) + IBAN (`NL03 TRBK ...`) + o.v.v. `<name>`. `POST /api/clients/[id]/payment-request` (admin, vereist e-mail) verstuurt en zet `paymentRequestSentAt`. Op de admin-klantpagina staat bij **event-klanten mét e-mail** een knop **"Stuur betaalverzoek"** + status. Hiervoor is het **e-mail- én aanhef-veld nu ook beschikbaar voor events** (in `/admin/clients/new` + inline bewerken; create-API sloeg `email` al op, alleen het formulier gatete het). Voor events = e-mail/aanhef van de organisator. Mail staat ook in `/admin/mail-preview`. (NB: betaalverzoek-endpoint werkt technisch voor elke klant met e-mail, maar de knop staat alleen bij events; personal heeft de betaalregel al in de oplever-mail.)
- Review-mail aanhef via contactName (juli 2026): de review-vraag (`reviews.ts`) gebruikte een eigen `reviewFirstName()` die altijd het eerste woord van de albumnaam pakte (album "Feest Mick & Marieke" → "Hoi Feest,"). Nu gebruikt 'ie dezelfde `greetingName()` als de andere klant-mails, dus `contactName` indien gezet ("Hoi Mick & Marieke,"), anders eerste woord van `name`. `greetingName()` is daarvoor van `clientMail.ts` naar de gedeelde `email.ts` verhuisd (geëxporteerd) en `PendingReviewClient` draagt nu `contactName` mee. `buildReviewHtml`/`buildReviewText` nemen nu de al-opgeloste aanhef-string i.p.v. de naam (mail-preview blijft werken). Geen data-migratie nodig; het `contactName`-veld bestond al.
- Prijsmodel foto's omgezet naar prijs-per-foto ladder (juli 2026, `eventPackages.ts`): de oude "greedy pakket-combinatie" (single/pack3/pack5) gaf een knik - 4 foto's (€17) was duurder per foto dan 3 (€12) en bijna gelijk aan 5 (€18). Nu een **aflopende marginale prijs per foto** zodat de prijs per foto altijd daalt. Event: foto 1 €5, foto 2 €4, foto 3 t/m 7 €3, vanaf foto 8 €2 (→ 3=€12, 5=€18 blijven; 4=€15). Personal: foto 1 €10, foto 2 €8, foto 3 t/m 5 €7, vanaf foto 6 €6 (→ 3=€25 blijft; 5=€39). `calculatePriceForCount(n, tier)` telt nu de marginale tarieven op i.p.v. pakketten te fitten. `priceForLegacyPackage` verwijderd (was ongebruikt). Nieuwe `priceRule(tier)` voor de uitlegregel in de winkelmand; `priceCatalog` toont nu voorbeeldpunten (1/3/5/8). `priceForUnlimited` (legacy €25) blijft. Server berekent nog steeds server-side op basis van `client.type` (event/personal).
- Review-verzoek per bestelling (juli 2026): event-kopers zijn `Order`s (met `customerEmail`), geen `Client`s, dus de review-cron miste ze. Nu op de orders-adminpagina (`/admin/orders`) een knop **"Stuur review-verzoek"** die alleen verschijnt zodra de order op `paid` of `shipped` staat (en er een e-mail is). Nieuwe route `POST /api/orders/[id]/review-request` (admin) stuurt de bestaande review-mail naar `customerEmail` (met een event-passend onderwerp "Bedankt voor je foto's...") en zet `reviewRequestedAt` op de order. `Order` heeft daarvoor het nieuwe veld `reviewRequestedAt?`. De review-mail-kern is uitgefactord naar `sendReviewRequestTo({ name, contactName?, email, subject? })` in `reviews.ts`, hergebruikt door zowel de klant-cron als deze knop. Knop toont "Review opnieuw" + een groene "verstuurd op <datum>"-regel zodra 'ie een keer verstuurd is. Handmatig (geen cron) omdat de levering ook handmatig gaat.
...
- Facturen (augustus 2026): de portal kan een echte factuur uitschrijven, printbaar / op te slaan als PDF. **Alle vaste gegevens staan in `src/lib/invoiceSettings.ts`**: `INVOICE_SENDER` (naam, handelsnaam, adres, e-mail, telefoon, IBAN, btw-nummer), `PAYMENT_TERM_DAYS` (14) en `VAT_RATE`.
  - **Fiscale situatie (belangrijk voor de context)**: BJAY heeft **geen KVK-inschrijving** maar staat als particulier bij de Belastingdienst geregistreerd en valt onder de **reguliere btw-plicht, dus niet onder de KOR**. Daarom staat er wél een **btw-identificatienummer** (`NL005449758B10`) op de factuur en **geen KVK-nummer** — dat veld bestaat niet meer in `InvoiceSender`. De eerste opzet ging nog uit van geen btw ("Geen btw in rekening gebracht", opbrengst als resultaat uit overige werkzaamheden); dat is eind augustus 2026 omgezet naar 21% btw.
  - **Btw-opbouw**: `Client.price` en `Invoice.amount` zijn het bedrag **exclusief** btw. `createInvoice` rekent `vatAmount = round(amount * VAT_RATE, 2)` en `totalIncl`, en legt `vatRate` mee vast in de momentopname, zodat een tariefwijziging oude facturen niet raakt. Het vel toont drie regels: subtotaal excl. btw, "Btw 21%", totaal te betalen incl. btw. De betaalregel op het vel noemt het bedrag **inclusief** btw; de facturenlijst telt op excl. btw (dat is de omzet) met btw en incl. als bijregel.
  - **Toon van de factuurmail (september 2026)**: was "Graag betalen voor \<datum\>", wat als een aanmaning las. Nu een bedankje vooraf (zelfde toon als het betaalverzoek) en een **vraag** i.p.v. een opdracht: "Zou je dat bedrag voor \<datum\> willen overmaken naar ..., met factuurnummer \<nr\> erbij?". De bedrag-zin noemt **beide** bedragen — "Het gaat om €500,- exclusief btw; met 21% btw komt het totaal op €605,-" — met het **totaal als laatste**, want anders maakt een boekhouder het exclusieve bedrag over. Zit in `amountSentence()` in `invoiceMail.ts`, gedeeld door de HTML- en tekstversie via een `bold`-callback (in HTML `<strong>`, in tekst kaal).
  - **Legacy-facturen**: facturen van vóór de omzetting hebben geen `vatRate` maar wel het oude `vatNote`. `InvoiceSheet` valt daarop terug (één totaal + de oude tekstregel) zodat ze renderen zoals ze destijds zijn uitgeschreven. Nieuwe facturen zetten `vatNote` niet meer. **Let op**: zulke facturen kloppen fiscaal niet meer nu er btw-plicht is; die moeten opnieuw worden uitgeschreven (of gecrediteerd als ze al verstuurd zijn).
  - **Datamodel**: nieuwe `Invoice` + `InvoiceSender` in `types.ts`. Een factuur is bewust een **volledige momentopname** (bedrag, omschrijving, klantnaam + adres, afzender); wijzig je daarna `Client.price` of `invoiceSettings.ts`, dan verandert een al uitgeschreven factuur niet mee. `Client` heeft er twee velden bij: `invoiceAddress` (factuuradres van de klant, vrije tekst met regelovergangen; in te vullen bij aanmaken én via inline bewerken) en `invoiceNumber` (backlink, zodat de klantpagina weet dat er al een factuur ligt).
  - **Nummering**: `invoice:counter:<jaar>` via Redis `INCR`, formaat `2026-001` (doorlopend per kalenderjaar, zero-padded). Atomair, dus nooit twee keer hetzelfde nummer; faalt het aanmaken daarna alsnog dan ontstaat een gat, en dat is uitlegbaarder dan een dubbeling. Redis: `invoice:<nummer>` + set `invoices:all`. Logica in `src/lib/invoices.ts` (`createInvoice` / `getInvoice` / `listInvoices`).
  - **Routes**: `POST /api/clients/[clientId]/invoice` (admin) schrijft de factuur uit en zet de backlink; weigert een tweede factuur voor dezelfde klant (409). `GET` op dezelfde route geeft de factuur van die klant. `GET /api/invoices` (lijst, met per factuur de `paidAt` van de bijbehorende klant) en `GET /api/invoices/[number]`.
  - **Pagina's**: `/admin/invoices` (lijst + totaal gefactureerd, knop "Facturen" op het dashboard), `/admin/invoices/[number]` (het vel + "Print / opslaan als PDF") en `/admin/invoices/preview` (voorbeeldfactuur met test-data, in de geest van `/admin/mail-preview`; slaat niets op en verbruikt geen nummer — hiermee check je de opmaak). Gedeeld vel in `src/components/InvoiceSheet.tsx`, inclusief `INVOICE_PRINT_CSS`.
  - **Printen**: geen PDF-library. `window.print()` + `@media print`; alles met className `no-print` valt weg en de wrapper `invoice-page` wordt platgeslagen. Dat laatste is nodig omdat `max-w-3xl` (768px ≈ 203mm) breder is dan de 174mm die binnen de A4-marges past — zonder die reset schaalt de browser de hele factuur kleiner. Gemeten op 658px (A4 minus 18mm marges): vel 658px breed, geen overflow. Het logo staat op `priority` (niet lazy), anders kan 'ie ontbreken op de afdruk.
  - **Vel vult de pagina**: het vel is `flex flex-col` met `minHeight: 254mm` en de voettekst (contact + IBAN + t.z.t. KVK/btw) heeft `mt-auto`, zodat die altijd onderaan de A4 staat i.p.v. direct onder de inhoud. 254mm i.p.v. de volle 261mm printhoogte is bewust: met de exacte maat kan een afrondingsverschil een lege tweede pagina opleveren. De voettekst is een `justify-between`-rij: contactgegevens links, de slogan **"Geen filter, wel gevoel"** rechts uitgelijnd ernaast (10px, goud, uppercase, tracking 0.1em, 154px breed). Schrijfwijze aangehouden van de live site (komma, geen punten). Let op als KVK- en btw-nummer erbij komen: de contactregel is dan breder en wikkelt naar een tweede regel; door `items-end` blijft de slogan dan onderaan uitlijnen.
  - **Versturen (augustus 2026)**: de factuur gaat als **PDF-bijlage** de deur uit vanuit de app, maar de PDF wordt **niet server-side gegenereerd**. Workflow: printknop -> opslaan als PDF -> op dezelfde pagina in het blok "Verstuur factuur" het e-mailadres (voorgevuld uit `Invoice.customerEmail`, overschrijfbaar) en het opgeslagen bestand kiezen -> `POST /api/invoices/[number]/send` mailt 'm. Zo leeft de opmaak op één plek (de browser rendert 'm, geen tweede layout in `pdf-lib` en geen headless Chrome op Vercel). De route valideert: geldig e-mailadres, base64 begint met `JVBER` (= `%PDF`, vangt af dat je per ongeluk een JPG kiest), max 3 MB decoded (Vercel's request-body-limiet is 4,5 MB en base64 blaast ~33% op). `sendBrandedMail` in `email.ts` heeft daarvoor een optionele `attachments`-parameter gekregen (Resend accepteert `[{filename, content}]` met base64 rechtstreeks in de REST-call). Mailtekst staat in `src/lib/invoiceMail.ts` (`invoiceBodyHtml` / `invoiceBodyText` / `sendInvoiceMail`) — bewust een **standaardtekst zonder persoonlijk bericht**, keuze van BJAY: aanhef via de gedeelde `greetingName()`, bedrag, vervaldatum, IBAN en factuurnummer. Staat ook in `/admin/mail-preview`. `Invoice` heeft er twee velden bij: `customerContactName` (aanhef, snapshot van `Client.contactName`) en `sentAt` — dat laatste is het **enige** veld dat na het uitschrijven nog verandert.
  - **Bewust niet gebouwd**: geen server-side PDF-generatie (zie hierboven waarom niet). Geen meerdere factuurregels (één regel uit `Client.price`, omschrijving overschrijfbaar). Geen losse facturen buiten een klant om. En de factuur raakt het **omzet-overzicht niet**: `Client.paidAt` blijft de enige bron voor "ontvangen", de facturenlijst leest die status alleen uit. Alle drie expliciete keuzes van BJAY.
