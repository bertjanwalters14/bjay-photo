...
- Thumbnails via Cloudinary i.p.v. Vercel-optimizer (juni 2026): bij een foto-zware galerij gaf het rooster lege (klikbare) vlakken terwijl de modal de foto wél toonde. Oorzaak: `PhotoGrid` + admin-grid gebruiken `next/image`, dat door Vercel's image-optimizer liep en tegen de optimalisatielimiet aanliep (kapotte thumbnails). De modal werkt omdat die een plain `<img src={photo.url}>` direct van Cloudinary laadt. Fix: `images.unoptimized: true` in `next.config.ts` — Cloudinary optimaliseert al (q_auto/f_auto/breedte in de URL), dus Vercel hoeft er niet overheen. Alle thumbnails laden nu direct van Cloudinary; sneller, geen Vercel-quota meer, en het valt niet meer om bij veel foto's. (`remotePatterns` voor res.cloudinary.com staat er nog, schaadt niet.)
